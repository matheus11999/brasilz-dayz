...
modded class SCR_PlayerController
{
	...
	protected void BZ_SinkCharacterOnDisconnect(IEntity character)
	{
		...
		// Use Teleport (replicated, child-aware) instead of SetOrigin. SetOrigin in the
		// previous version was the root cause of weapon-attachment corruption — moving the
		// root without updating attached weapon transforms left stale state in the inventory
		// item components, which then desynced on reconnect.
		vector transform[4];
